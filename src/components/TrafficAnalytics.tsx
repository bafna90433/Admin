...
// ✅ State code → State name (geoip-lite returns codes like MH, GJ etc.)
...
const getStateName = (code: string) => INDIA_STATE_MAP[code] || code || "Unknown";
...
const TrafficAnalytics: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const sortedStats = stats.sort(
        (a: TrafficData, b: TrafficData) =>
          new Date(a.date).getTime() - new Date(b.date).getTime()
      );
      setDailyStats(sortedStats);

      setSourceStats(
        res.data.sourceStats || {
          google: 0,
          instagram: 0,
          facebook: 0,
          whatsapp: 0,
      ...
      // ✅ NEW: states
      setStateStats(res.data.stateStats || {});
    } catch (error) {
  ...
  // ✅ Avg Daily for last 7 days (not 30)
  const avgDaily = dailyStats.length > 0 ? Math.round(totalVisitors / 7) : 0;
      ...
      <header className="dashboard-header" style={{ marginBottom: "20px" }}>
        <div>
          ...
          <p className="text-muted">
            Detailed report of your store&apos;s visitors and performance.
          </p>
        </div>
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card border-blue">
          <div className="stat-info">
            ...
            <h3 className="stat-value">{totalVisitors.toLocaleString()}</h3>
          </div>
        ...
        <div className="stat-card border-purple">
          <div className="stat-info">
            <span className="stat-title">Visits Today</span>
            <h3 className="stat-value">{todayVisits}</h3>
          </div>
        ...
        <div className="stat-card border-orange">
          <div className="stat-info">
            <span className="stat-title">Avg. Daily</span>
            <h3 className="stat-value">{avgDaily}</h3>
          </div>
                      ...
                      >
                        <div
                          ...
                          style={{
                            width: "100%",
                            backgroundColor:
                              data.date === todayDate ? "#10b981" : "#3b82f6",
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
            ...
            <section className="card">
              ...
              <div style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "15px" }}>
                ...
                <SourceRow
                  icon={<FiGlobe />}
                  label="Direct / Other"
                  count={(sourceStats?.direct || 0) + (sourceStats?.other || 0)}
                  color="#718096"
                />
              </div>
            </section>
            ...
            <section className="card">
              ...
              <div style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "12px" }}>
                {topStates.length === 0 ? (
                  <p className="text-muted text-center">No state data yet.</p>
                ) : (
                  topStates.map(([code, count]) => (
                    ...
                    >
                      <span style={{ fontWeight: 600, color: "#2d3748" }}>
                        {getStateName(code)}
                        <span style={{ marginLeft: 8, color: "#718096", fontWeight: 500 }}>
                          ({code})
                        </span>
                      </span>
                    ...
                    .map((data) => (
                      <tr key={data.date}>
                        <td style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                          ...
                          {data.date === todayDate && (
                            <span className="status-badge delivered" style={{ fontSize: "9px", padding: "2px 6px" }}>
                              Today
                            </span>
                          )}
                        </td>
