...
const generateInvoice = (order: Order) => {
  ...
  const paymentText = order.paymentMode === "ONLINE" ? "Online (Paid)" : "COD";
    ...
    <body>
      <div class="invoice-container">
        ...
        <div class="invoice-details">
          ...
          <div class="detail-section"><h3>Ship To</h3><p>${shippingHtml}</p></div>
          <div class="detail-section">
             <h3>Order Details</h3>
             <p>Invoice: ${order.orderNumber}<br>Date: ${currentDate}<br>Payment: ${paymentText}<br>${order.trackingId ? `AWB: ${order.trackingId}` : ''}</p>
          </div>
        </div>
        <table>
...
const AdminOrders: React.FC = () => {
  ...
  const shipOrderHandler = async (orderId: string) => {
    ...
    try {
      ...
      setOrders(prev => prev.map(o => o._id === orderId ? { ...o, isShipped: true, trackingId: data.waybill, courierName: 'iThinkLogistics', status: 'shipped' } : o));
      if (viewing?._id === orderId) {
        setViewing(prev => prev ? { ...prev, isShipped: true, trackingId: data.waybill, courierName: 'iThinkLogistics', status: 'shipped' } : null);
      }
    } catch (error: any) {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return list.filter(o => {
      ...
      const inCustomer = norm(o.customerId?.shopName).includes(q) || norm(o.customerId?.otpMobile).includes(q);
      ...
      <div className="admin-container">
        ...
        <div className="admin-controls">
          ...
          <div className="search-box">
            ...
            <input
              type="text"
              placeholder="Search ID, Shop, City..."
              value={search}
        ...
        <div className="modal-backdrop" onClick={() => setViewing(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-toolbar">
               <button className="btn-secondary" onClick={() => exportSingleOrder(viewing)}>⬇ Excel</button>
               <button className="btn-secondary" onClick={() => generateInvoice(viewing)}>📄 Invoice</button>
               {viewing.isShipped && viewing.trackingId && (
                 <div className="tracking-box">🚚 {viewing.courierName}: <b>{viewing.trackingId}</b></div>
               )}
               <button className="btn-danger" onClick={() => deleteOrder(viewing._id)}>🗑 Delete</button>
            </div>
               ...
               <div className="modal-section">
                  ...
                  {viewing.shippingAddress ? (
                    <>
                      ...
                      <p>{viewing.shippingAddress.street}, {viewing.shippingAddress.area}</p>
               ...
               {viewing.items.map((it, i) => (
                 <div className="item-row" key={i}>
                    ...
                    <div className="item-details">
                       ...
                       <span className="item-meta">{toInners(it)} packets • x{it.qty} units</span>
                    </div>
